...
object NotificationPlayer {
    ...
    fun createNotification(context: Context, filename: String, manager: NotificationManager, playPause: Boolean): Notification {
        ...
        // custom view
        val remoteViews = RemoteViews(getPackageName(OnAir::class.java), R.layout.notificatoin_player)
        remoteViews.setImageViewResource(R.id.img_albumart, R.drawable.ic_radio_antenna)

        // true -> playing
        if ( playPause ) {
            remoteViews.setTextViewText(R.id.txt_title, "재생중: " + RadioChannelResources.getDefaultButtonTextByFilename(
                filename
            ))
            ...
            remoteViews.setTextViewText(R.id.txt_title, "일시정지: " + RadioChannelResources.getDefaultButtonTextByFilename(
                filename
            ))
